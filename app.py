...
TRANSLATIONS = {
    "en": {
        ...
        "intro": "Please answer 4 simple questions to let the we design your perfect meal.",
        ...
        "q2_opts": [
            "a. Always Hot / Sweating ",
            "b. Normal",
            "c. Always Cold / Chills ",
        ],
    ...
    "vi": {
        ...
        "intro": "Hãy trả lời 4 câu hỏi đơn giản để chúng tôi thiết kế bữa ăn phù hợp nhất với thể trạng của bạn.",
        "serving": "Đang phục vụ Khách hàng:",
        "q1": "1. Giấc ngủ gần đây của bạn thế nào?",
        "q1_opts": [
            "a. Khó ngủ / Trằn trọc ",
            "b. Bình thường / Ngủ ngon",
            "c. Ngủ li bì / Vẫn mệt ",
        ],
        "q2": "2. Bạn cảm thấy thân nhiệt cơ thể thế nào?",
        "q2_opts": [
            "a. Hay nóng / Đổ mồ hôi ",
            "b. Bình thường",
            "c. Hay lạnh / Sợ gió ",
        ],
        "q3": "3. Trạng thái cảm xúc hiện tại?",
        "q3_opts": [
            "a. Dễ cáu gắt / Nóng tính ",
            "b. Bình tĩnh / Thư thái",
            "c. Buồn chán / Ít năng lượng ",
        ],
        "q4": "4. Hiện tại bạn cảm thấy thế nào?",
...
lang_option = st.sidebar.radio("Choose your language:", ("🇬🇧 English", "🇻🇳 Tiếng Việt"))
...
def calculate_score(survey_indices):
    ...
    for category, idx in survey_indices.items():
        ...
        if idx == 0:
            reasons.append(f"{T['yang']}")
        if idx == 2:
            reasons.append(f"{T['yin']}")
    ...
    if not matches:
        ...
        for dish in matches:
            with st.container():
                ...
                with col_stat:
                    # =======================================================
                    # FIXED: Using valid Streamlit Shortcodes for Icons
                    # =======================================================
                    if dish["val"] > 0:
                        # Yang (Heat)
                        icon_str = f"🔥 :fast_forward: (+{dish['val']})"
                    elif dish["val"] < 0:
                        # Yin (Cold)
                        icon_str = f"❄️ :rewind: ({dish['val']})"
                    else:
                        # Neutral
                        icon_str = f"⚖️ :heavy_minus_sign: (0)"

                    st.success(f"{T['dish_impact']}: {icon_str}")
